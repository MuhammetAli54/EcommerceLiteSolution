...
namespace EcommerceLiteBLL.Repository
{
    ...
    public class CategoryRepo:RepositoryBase<Category,int> 
    {
        public List<ProductCountModel> GetBaseCategoriesProductCount()
        {
            ...
            var categoryList = from c in dbContext.Categories where c.BaseCategoryId == null select c;

            foreach (var item in categoryList)
            {
                var subCategoryList = from c in dbContext.Categories where c.BaseCategoryId == item.Id select c;
                int productCount = 0;
                foreach (var subitem in subCategoryList)
                {
                    var productList = from p in dbContext.Products where p.CategoryId == subitem.Id select p;
